...
class Grid {
	...
	void plotDiagonal(Line &line) {
		cout << "Plot Diagonal" << endl;	
		int x = line.getX1();
		int direction_x = line.getX1() < line.getX2() ? 1 : -1;
		int end_x = line.getX2();
		int y = line.getY1();
		int direction_y = line.getY1() < line.getY2() ? 1 : -1;
		_measures[y][x] += 1;
		do {
			x += direction_x;
			y += direction_y;
...
int main(int argc, char *argv[]) {
	...
	if (argc != 2) {
		cout << "Usage: day5b <input.txt>" << endl;
		return -1;
	}
